...
Doom Emacs overrides the default whitespace-mode behavior to display errors, which is useful, but sometimes I want to use it to inspect the buffer's whitespace. This snippet allows me to toggle between three whitespace-mode behavior states: Doom's default, original behavior, and off.
...
Avy is the best way to jump around in Emacs. Enable all windows as potential targets. To make returning after jumping with Avy easier, also configure [[help:pop-global-mark][pop-global-mark]] to switch windows if possible.
...
Doom provides many enhancements for the vanilla [[help:visual-line-mode][visual-line-mode]] experience via [[help:+word-wrap-mode][+word-wrap-mode]]. However, it does not provide a nice way to toggle between word wrapping and truncating, so I implementated the feature myself. I disable it by default beacause I prefer truncating in programming modes, which is where I spend the majority of my time.

#+begin_src emacs-lisp
(defvar my-word-wrap-enabled nil)
#+end_src
